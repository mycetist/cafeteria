...
from app.models import (
    ...
    MealRecord, PurchaseRequest, PurchaseItem, Notification, Allergy, Review, DishPurchase, Subscription
)
...
@cook_bp.route('/meals/search-purchases', methods=['GET'])
@cook_required
def search_purchases():
    try:
        query = request.args.get('q', '')
        meal_type_filter = request.args.get('meal_type', None)
        
        if not query or len(query) < 1:
            return jsonify({'error': 'Поисковый запрос должен быть не менее 1 символа'}), 400

        today = date.today()
        query_lower = query.lower()
        
        is_numeric_query = query.strip().isdigit()
        query_id = int(query.strip()) if is_numeric_query else None
        
        purchase_query = DishPurchase.query.filter(
            db.func.date(DishPurchase.purchase_date) == today,
            DishPurchase.is_used == False
        )
        
        if meal_type_filter:
            purchase_query = purchase_query.filter_by(meal_type=meal_type_filter)
        ...
        for purchase in purchases:
            ...
            if is_numeric_query and user.id == query_id:
                pass
                ...
                if query_lower not in full_name and query_lower not in email:
                    continue
            ...
            purchase_data['user']['allergies'] = [a.allergy_type for a in allergies]
            
            dish = Dish.query.get(purchase.dish_id)
            if dish:
                purchase_data['dish'] = dish.to_dict()
...
def create_purchase_request():
    ...
    for item in items:
        ...
        try:
            ...
            if quantity > 9999999.99:
                return jsonify({'error': 'Количество слишком большое (максимум 9999999.99)'}), 400
        except ValueError:
            return jsonify({'error': 'Неверное значение количества'}), 400
    ...
    for admin in admins:
        notification = Notification(
            ...
            message='Создана новая заявка на закупку'
        )
        db.session.add(notification)
