...
def create_subscription():
    ...
    if existing:
        ...
        message = f'Абонемент продлён до {existing.end_date}. Всего блюд: {existing.meals_remaining}'
    else:
...
def purchase_dish(dish_id):
    ...
    if menu_id:
        ...
        if menu.menu_date != today:
            if menu.menu_date < today:
                return jsonify({'error': 'Нельзя покупать блюда из прошедших меню'}), 400
            else:
                return jsonify({'error': 'Нельзя покупать блюда из будущих меню. Подождите до этой даты'}), 400
    ...
    payment = Payment(
        ...
        payment_type='single',
        status='completed',
        transaction_id=f"DISH{datetime.utcnow().strftime('%Y%m%d%H%M%S%f')}{user_id}"
    )
    db.session.add(payment)
